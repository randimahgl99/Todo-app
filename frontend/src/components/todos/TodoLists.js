...
function TodoLists({fetchUserTodos , userTodos, setUserTodos, BASE_URL}) {
  ...
  return (
    ...
    <div>
      <form className='todoForm flex flex-row items-center justify-between border-sold border-2 w-[50rem] h-[5rem] rounded-[1rem] bg-[white] mb-[2rem]  'onSubmit={handleSearch} >
    <div>
  <input type="text" placeholder="Enter to search" className='w-[10rem] m-[1rem] px-[1rem] rounded-[0.5rem] border-sold border-2 placeholder:italic  placeholder:relative placeholder:left-[0.1rem] placeholder:ml-4 h-[3rem] focus:outline-none focus:ring-[0.3rem] focus:ring-green-700 '
    value={search}
  onChange={(e)=>{setSearch(e.target.value)}}
  ></input>
  <button type='submit' className='text-[#242B2E] bg-[#CAD5E2] px-[3rem] py-[0.5em] rounded-[0.5rem] pointer-cursor font-bold active:bg-green-700 active:text-white'  >Search</button>
  </div>
...
<option value="updation">By Updation</option>
    ...
    <div className='shadow-md bg-slate-200/100  w-[50rem] mx-auto mt-[1rem] rounded-[1rem] pb-[1rem]'>

      
      {userTodos && userTodos.map((user)=>{
          
       return <div className='overflow-hidden  ml-6 rounded-[1rem] '>
      {/* title bar */}
      ...
      <p className='p-[0.1rem] inline-block cursor-pointer text-[#28522d] font-bold' key={user._id} >{user.Title}</p>
      ...
      <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
        ...
        <button className='bg-gray-400 active:bg-gray-400 px-[0.8rem] rounded-[0.3rem]'
              
          onClick={()=>{handleEditTitle(user)}}
         
        >Edit</button>
        <button className='bg-gray-400 active:bg-gray-200 px-[0.8rem] rounded-[0.3rem]' 
          onClick={()=>{
      ...
      <div className='arrayTasks bg-slate-200/100 max-h-0 peer-checked:max-h-screen  '>

                  {/* Adding Tasks inside title */}
          <div className='flex flex-row items-center justify-start ml-[1rem] h-[1.5] space-x-[1.5rem] '>
            <div>
            <input type="text" placeholder='Enter tasks' className='mt-[1.5rem] w-[30rem] h-[2.5rem] rounded-[0.3rem]  px-[1rem] focus:outline-none focus:ring-[0.1rem] focus:ring-gray-500 placeholder:italic border-solid  '
            value={tasks}
            ...
            {/* Tasks inside title */}
             ...
             <div>
             <p className="p-[0.2rem] ml-[1.8rem]" key={user._id} >{tasks}</p>
             </div>
             <div  className='flex flex-row items-center justify-between mr-[2rem] space-x-[2rem]'>
             <button className='bg-gray-400 active:bg-gray-400 px-[0.8rem] rounded-[0.3rem]' onClick={()=>{handleEditTaskForTitle(user,index)}}  >Edit</button>
             <button className='bg-red-400  active:bg-red-400 px-[0.8rem] rounded-[0.3rem]' onClick={()=>{
